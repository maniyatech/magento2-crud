...
namespace ManiyaTech\Crud\Model;
...
{
    public const BASE_PATH = 'ManiyaTech/Profile/';
    ...
    public function getData(): array
    {
        foreach ($this->collection->getItems() as $model) {
            $data = $model->getData();
            if ($model->getHobby()) {
                $data['hobby'] = explode(",", $model->getHobby());
            }
            if ($model->getProfile()) {
                $data['profile'] = $this->prepareImageData($model->getProfile());
            }
